...
Пользователю: начните с [пошагового руководства](user-guide.md) — установка, первая презентация, обновление и решение ошибок. Ниже описаны технические возможности среды и профили клиентов.
...
Профили установки проверяются файловыми тестами. Это не означает, что каждый продукт был запущен и каждая модель проверена. Сборка PPTX и автоматические тесты portable прошли на Linux (Python 3.9), Windows и macOS (Python 3.12): [CI от 2026-09-25](https://github.com/TnockTnock/CHU_presentantion_skill/actions/runs/36122590598). В CI прошли 27 тестов, 3 проверки ранее созданных локальных файлов пропущены; локально прошли все 30. Визуальная проверка PDF/PNG выполнена на macOS; рендер на Windows/Linux отдельно не проверен. Отсутствие API-вызовов моделей делает сборщик независимым от модели, однако качество структуры и фактологии остаётся ответственностью агента. Не обещай одинаковое качество всем моделям.
