...
KINDS = {'cover', 'cards', 'kpi', 'process', 'diagram', 'chart', 'table', 'text'}
...
def validate(deck):
    ...
    require(not (set(deck) - {'schema_version','title','demo','sources','slides'}), 'Unknown root fields')
    ...
    for i, s in enumerate(slides, 1):
        ...
        require(not (set(s) - {'id','kind','title','source_ids','notes','footer'} - extra[kind]), prefix + ': unknown fields')
        ...
        if kind == 'cover':
            ...
            for edge in edges:
                ...
                require(not (set(edge) - {'from','to','from_side','to_side'}), prefix + ': unknown edge fields; labels are not supported yet')
